...
function readValidation() {
  ...
  // Matched against the DOCUMENT's wording, which differs from the harness's console
  // summary — the console says "correct on 4 of 5", the document says "4 of 5 injected
  // faults were classified correctly". Parsing the document is right, because that is the
  // artefact that gets committed and read; the console output is not kept.
  const classHit = /\*\*(\d+) of (\d+) injected faults were classified correctly/i.exec(text);
  const classBaseline = /always\s+answered\s+`?[\w-]+`?\s+would score (\d+) of (\d+)/i.exec(text);

  // The pooled lead-time row: how far ahead of the failure the first warning arrived, across
  // every injected fault. Median and tenth percentile, in days.
  const pooled = /\|\s*\*\*ALL FAULTS POOLED\*\*\s*\|\s*\*\*(\d+)\*\*\s*\|\s*\*\*([\d.]+)\*\*\s*\|\s*\*\*([\d.]+)\*\*\s*\|\s*\*\*([\d.]+)\*\*/i.exec(text);

  /*
   * The remaining-life interval coverage, and it is the WEAK result in this report — the
   * bands are nominally 80% and cover far less than that in practice. It is pulled out
   * deliberately rather than left buried: a walkthrough that shows the flattering numbers
   * and quietly omits this one is doing the thing this whole project argues against.
   */
  const coverage = /coverage against that crossing is ([\d.]+)%/i.exec(text);
  ...
  return {
    ...
    faultClassBaseline: classBaseline ? Number(classBaseline[1]) : null,
    leadWarnings: pooled ? Number(pooled[1]) : null,
    leadMedianDays: pooled ? Number(pooled[2]) : null,
    leadP10Days: pooled ? Number(pooled[3]) : null,
    leadWorstDays: pooled ? Number(pooled[4]) : null,
    rulCoverage: coverage ? Number(coverage[1]) : null,
    suppressionRefusals: (text.match(/CORRECT REFUSAL/g) ?? []).length,
    suppressionWrong: (text.match(/WRONG ATTRIBUTION/g) ?? []).length,
    generatedAt: (/Generated by[^\n]*?on ([0-9]{4}-[0-9]{2}-[0-9]{2})/i.exec(text) ?? [])[1] ?? null,
  };
}
